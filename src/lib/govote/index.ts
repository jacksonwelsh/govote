...
async function propagate(
  ...
  petitionId: ObjectId,
  votingOptions: votingOptions = { answerIndex: 0, representative: voterId }
) {
...
export async function vote(
  ...
  const voter = collections.voters.findOne({ id_: voterId });
  assert.notStrictEqual(voter, undefined, "Couldn't find a voter with that id");
  ...
  if (voterId != votingOptions.representative && petitionsVotedFor?.includes(petitionId)) {
    assert.notStrictEqual(
      petition.votes[voterId],
      undefined,
      "The voter says it voted but the petition doesn't"
    );
    ...
    if (
      voter.representatives.indexOf(petition.votes[voterId].representative) < voter.representatives
    ) {
  ...
  if (!voter.isGroup) {
    //Vote
    collections.petitions.updateOne(
      { _id: petitionId },
      {
...
export async function setRepresentatives(voterId: ObjectId, newRepresentatives: ObjectId[]) {
  ...
  const additions: ObjectId[] = newRepresentatives.filter((rep) => {
    return !oldRepresentatives?.includes(rep);
  });
  ...
  const removals: ObjectId[] = oldRepresentatives.filter((rep) => {
    return !newRepresentatives?.includes(rep);
  });
