<script>
	...
	export let _id;
		...
		.then((signaturesArray) => {
			...
			return fetch('/users/search.json?' + new URLSearchParams({query: '{}'}))
			//return fetch('/users/search.json?' + new URLSearchParams({query: JSON.stringify({_id: { $in : signaturesArray }})}))
